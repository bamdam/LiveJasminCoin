...
// Copyright (c) 2011-2013 LiveJasminCoin Developers.
...
static CBigNum bnStartDiff(~uint256(0) >> 28);
...
const string strMessageMagic = "LiveJasminCoin Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 999000); 

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan =  .1 * 24 * 60 * 60; // LiveJasminCoin: 0.1 days
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
    ...
    // LiveJasminCoin: This fixes an issue where a 51% attack can change difficulty at will.
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal LiveJasminCoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "LiveJasminCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc1, 0xaf, 0xd9 }; 
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // LiveJasminCoind: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
